...
func (p *parser) Init() {
	...
	_rules = [...]func() bool{
		...
		nil,
		/* 5 primary <- <((&('A' | 'a') agent) | (&('(') ('(' ws Expr ws ')')) | (&('N' | 'n') (('n' / 'N') ('o' / 'O') ('t' / 'T') wsp primary Action2)))> */
		func() bool {
		...
		nil,
		/* 7 regexp <- <((char* '(' regexp ')' char*) / (char* '|' regexp*) / char+)> */
		func() bool {
		...
		nil,
		nil,
		/* 16 Action3 <- <{ p.Agent(buffer[begin:end]) }> */
		nil,
	}
	p.rules = _rules
}
